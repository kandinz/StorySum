...
description: Build Flutter Android ARM64 release APK (app-arm64-v8a-release.apk), upload it to GitHub Releases, push git changes, and run full index sync with Codebase Memory MCP.
---
...
1. **Chỉ build file `app-arm64-v8a-release.apk`** (tối ưu dung lượng và thời gian build cho thiết bị Android 64-bit).
2. **Tải file `app-arm64-v8a-release.apk` lên GitHub Release** tương ứng với phiên bản.
...
### 🔹 Bước 1: Build APK ARM64
Chạy lệnh Flutter chỉ biên dịch kiến trúc ARM64:
```powershell
flutter build apk --release --target-platform android-arm64 --split-per-abi
```
File APK đầu ra: `build/app/outputs/flutter-apk/app-arm64-v8a-release.apk`

---

### 🔹 Bước 2: Push APK lên GitHub Release
Tạo hoặc cập nhật GitHub Release cho tag version (lấy từ `pubspec.yaml`), sau đó upload asset `app-arm64-v8a-release.apk`.
...
- **Chỉ định tag và ghi chú release:**
  ```powershell
  python .agents/skills/build-release-apk/scripts/release.py --tag v1.0.3 --title "Release v1.0.3" --notes "Ghi chú cập nhật..."
  ```
- **Tùy chỉnh commit message:**
  ```powershell
  python .agents/skills/build-release-apk/scripts/release.py --commit-msg "feat: cập nhật tính năng mới"
  ```
- **Bỏ qua bước build nếu APK đã có sẵn:**
  ```powershell
...
## ⚠️ Lưu ý quan trọng
- **Cập nhật version**: Trước khi tạo release mới, hãy tăng phiên bản trong `pubspec.yaml` (ví dụ `version: 1.0.3+4`).
- **File APK**: File APK chỉ chứa mã ARM64-v8a, tương thích tối ưu với 99%+ thiết bị smartphone Android hiện đại.
